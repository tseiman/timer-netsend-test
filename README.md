...
## Overview
	...
	- alternatively different approaches to generate network UDP traffic either purly in user space, 
...
## Assumptions
    ...
    - to sattisfy system under test host the network interface for the FPGA board assuned to have IP 1.1.1.2/24
      The system under test hosts network interface itself assumed to have 1.1.1.9/24
...
## Setting up ETH Interface:
    ...
    - assuming meassurement Card has ETH MAC 01:02:03:04:05:06, and
      all ethernet packets are sent to this MAC will be counted & meassured

    - well knowing that working as root user is a risk, all commands are given as root user

    - in ./scripts folder is a script "setup-interface.sh" which does nessesary operations, however there is no security
      which checks that accedently a wrong ethernet interface is changed. Use it on your own risk.
